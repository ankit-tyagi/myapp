...
class FuelLogScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('${vehicle.name} - Fuel Log'),
      ),
      body: ValueListenableBuilder(
        valueListenable: Hive.box<FuelEntry>('fuel_entries').listenable(),
        builder: (context, Box<FuelEntry> box, _) {
          final entries = box.values.where((entry) => entry.vehicleId == vehicle.key).toList();
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  title: Text('${entry.fuelQuantity}L on ${DateFormat.yMMMd().format(entry.date)}'),
                  subtitle: Text(
